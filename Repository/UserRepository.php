...
namespace Fulgurio\LightCMSUserBundle\Repository;
...
class UserRepository extends EntityRepository
{
    ...
    public function findAllWithPagination($filters, $limit, $offset, $resultInArray = FALSE)
    {
        ...
        $query = $this->getEntityManager()->createQuery('SELECT u FROM FulgurioLightCMSUserBundle:User u ' . $where . ' ORDER BY u.username ASC')->setMaxResults($limit)->setFirstResult($offset);
        if (!empty($filters))
        {
    ...
    {
        $where = $this->getQueryFilter($filters);
        $query = $this->getEntityManager()->createQuery('SELECT COUNT(u) FROM FulgurioLightCMSUserBundle:User u' . $where);
        if (!empty($filters))
        {
            foreach ($filters as $filterKey => $filterValue)
            {
